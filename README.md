...
- **MOM:** ActiveMQ con 3 colas (`ColaPagoConfirmado`, `ColaSAM`, `ColaSHC`). Todas las comunicaciones MS-Compra ⇄ SAM/SHC/SaludPay pasan por allí.
...
| `SAM` | Spring Boot + JMS | Sistema Agenda Médica; `@JmsListener("ColaSAM")` |
| `SHC` | Spring Boot + JMS | Sistema Historias Clínicas; `@JmsListener("ColaSHC")` |
| `SNS` | Spring Boot | Simulador externo SuperIntendencia Nacional de Salud |
| `SaludPay-Back` | .NET 8 + EF Core + SQL Server | Pagos transaccionales; publica `ColaPagoConfirmado` con Apache.NMS.ActiveMQ |
...
[2] Cliente → SaludPay-SPA → SaludPay-Back (Kestrel, .32.100.111)
                                  ...
                                  │ PUBLISH ColaPagoConfirmado via Apache.NMS.ActiveMQ
                                  ▼
                          MS-Compra @JmsListener("ColaPagoConfirmado")
                                  │ marca PAGADA
                                  ├─► PUBLISH ColaSAM ──► SAM (.121) → MySQL
                                  ├─► PUBLISH ColaSHC ──► SHC (.121) → MySQL
                                  │ marca TERMINADA
...
| --- | --- | --- | --- |
| `ColaPagoConfirmado` | SaludPay-Back (.NET) | MS-Compra | `{ cedula, numeroCompra, valorPagado, fechaPago }` |
| `ColaSAM`  | MS-Compra | SAM | `{ numeroCompra, cedulaCliente, servicios[] }` |
| `ColaSHC`  | MS-Compra | SHC | `{ numeroCompra, persona{}, planes[] }` |
...
6. **Consola ActiveMQ** (`http://10.43.100.122:8161`): debes ver 1 mensaje en `ColaPagoConfirmado`, luego en `ColaSAM` y `ColaSHC`.
